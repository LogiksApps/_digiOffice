...
switch($_REQUEST["action"]) {
  ...
  case "comments":
    ...
    if($_SESSION['SESS_PRIVILEGE_ID']>ADMIN_PRIVILEGE_RANGE) {
      $whereRAW="(shared_with NOT IN ('*','private') AND (shared_with='@{$_SESSION['SESS_GROUP_NAME']}' || shared_with='@{$_SESSION['SESS_PRIVILEGE_NAME']}' || shared_with='{$_SESSION['SESS_USER_ID']}' || shared_with='@'))";
    }
